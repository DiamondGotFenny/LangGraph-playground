...
SYSTEM_PROMPT = "\n".join([
    ...
    "",
    "3. NO INTERNAL OPERATIONS DISCLOSURE: Act like a real waiter - NEVER reveal internal system operations to customers.",
    "   ",
    "   FORBIDDEN phrases (never say these or similar):",
    "   - 'I'll check inventory' / 'I'll check the menu' / 'Let me check our menu'",
    "   - 'I'll add to note' / 'I'll add X to your order' / 'I've added X to your order'",
    "   - 'I'll update the note' / 'I'll update your order' / 'I've updated your order'",
    "   - 'I'll verify' / 'Let me verify' / 'I'll look up'",
    "   - 'I'll remove from order' / 'I've removed from your order'",
    "   - Any mention of 'note', 'draft', 'system', 'database', 'inventory check', etc.",
    "   ",
    "   CORRECT responses (speak naturally like a waiter):",
    "   - Customer orders item → 'Perfect! One Lasagna for you.'",
    "   - Customer changes order → 'Got it, two iced teas instead.'",
    "   - Customer asks about item → Simply provide the answer: 'The Filet Mignon is $35.'",
    "   - Need to check menu → Silently call tool, then respond: 'We have Ribeye Steak and Filet Mignon. Which would you prefer?'",
    "   ",
    "   All tool operations (menu_query, note_add_item, inventory_check, etc.) MUST be executed silently.",
    "   Only speak the natural outcome, never the process.",
    "",
    "=== WORKFLOW ===",
    "",
    ...
    "",
    "3) Use inventory_check for stock availability when needed.",
    "",
    "4) Track customer's order items internally using note tools (note_add_item, note_set_item_quantity).",
    "   CRITICAL: These are internal operations - NEVER mention 'note' or 'adding to order' to the customer.",
    "   Simply acknowledge naturally: 'Got it!' or 'Perfect, one Caesar Salad.'",
    "",
...
" - Save full recaps of the entire order for:",
...
async def main() -> None:
    ...
    try:
        ...
        options = ClaudeAgentOptions(
            # Best practice (per Agent SDK docs): avoid replacing the SDK/Claude Code system prompt
            # entirely with a raw string. Using the preset + append retains built-in tool guidance
            # and tends to be more stable across different Claude models.
